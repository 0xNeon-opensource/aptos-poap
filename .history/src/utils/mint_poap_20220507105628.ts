...
export async function mintPOAP(accountAddress: string) {
    ...
    console.log(`User: ${accountAddress.address()}`)
    ...
    console.log(`User: ${await restClient.accountBalance(accountAddress.address())}`)
    ...
    await client.offerToken(root, accountAddress.address(), root.address(), collection_name, token_name, 1)
    ...
    console.log(`root's token balance: ${token_balance}`)
    token_balance = await client.getTokenBalance(accountAddress.address(), root.address(), collection_name, token_name)
